...
class MainActivity : AppCompatActivity() {
    ...
    private fun confriomPassword(text: String): Boolean {
        if (password2.text.toString().isEmpty()){
            Toast.makeText(this, "Please type in the password confirmation", Toast.LENGTH_LONG).show()
        }
        else if (password2.text.toString().equals(text)){
            password2.setBackgroundColor(Color.GREEN)
            return true
        }
        else
            Toast.makeText(this,"The password is not identical.", Toast.LENGTH_LONG).show()
        password2.setBackgroundColor(Color.RED)
        return false
    }
